...
import {MapNumber,Speed,pause_game,other_tools,host,Online} from '../Components/Settings'
...
const socket2 = io('http://10.11.9.1:1340', {extraHeaders:{
...
export default function PingPong()
{
  ...
  const ping1style = {
	...
	function sendpos()
	{
		if(host)
		{
			socket2.emit('send_user1_data',ping2ypos);
				// await	socket2.on('send_user1_data',(data)=> {
		...
		else
		{
			...
			// await socket2.on('send_user2_data',(data)=> {
				socket2.on('send_user2_data',(data)=> {
				console.log(data);
				setPing2YPos(data);
				const ping2 = document.getElementById("ping2");
				if(ping2 != null)
